"""Plotly-Abbildungen der ALNS-Demo: Karten (mehrfarbig je Route, wortgleich zur Schwester-Demo), Vergleichs-
Balken, Sweeps, Skalierung, PLUS ALNS-eigen: Verlaufskurve (aktuelle/beste Kosten über die Iterationen) und
Operator-Gewichte am Ende des Laufs. Achsen sind gesperrt (fixedrange), damit Touch-Geräte beim Scrollen nicht
zoomen."""
...
def build_comparison_bar(labels, improvements):
...
def build_progress(cost_history, best_history):
    """Aktuelle Kosten (angenommene Lösung) und beste je gefundene Kosten über die Iterationen - zeigt sichtbar,
    wie die Metropolis-Akzeptanz zeitweise Verschlechterungen zulässt, während `best` monoton fällt."""
    xs = list(range(len(cost_history)))
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=xs, y=cost_history, mode="lines", line=dict(color=TOUR_COLOR, width=1.2), name="Aktuell (angenommen)", opacity=0.6))
    fig.add_trace(go.Scatter(x=xs, y=best_history, mode="lines", line=dict(color=BEST_COLOR, width=2.5), name="Bisher bestes"))
    fig.update_xaxes(title_text="Iteration")
    fig.update_yaxes(title_text="Kosten")
    return _base(fig, 360)


def build_weights_bar(weights, labels):
    names = list(weights.keys())
    fig = go.Figure(go.Bar(x=[labels[n] for n in names], y=[weights[n] for n in names], marker_color=IMPROVE_COLOR))
    fig.update_xaxes(title_text="")
    fig.update_yaxes(title_text="Gewicht am Ende des Laufs")
